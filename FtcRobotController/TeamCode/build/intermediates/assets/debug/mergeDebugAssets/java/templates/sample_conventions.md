...
With â€œSensorâ€? and â€œHardwareâ€? samples, the code should demonstrate the essential function,
and not be embellished with too much additional â€œcleverâ€? code.  If a sensor has special
addressing needs, or has a variety of modes or outputs, these should be demonstrated as
simply as possible.

Special programming methods, or robot control techniques should be reserved for â€œConceptâ€? Samples,
and where possible, Samples should strive to only demonstrate a single concept,
eg: State machine coding, or a User Menu system, and not combine them into a single â€œall inclusiveâ€?
sample.  This will prevent an â€œall inclusiveâ€? Sample being deleted just because one part of it
becomes obsolete.
...
Device Object names should use the same words as the deviceâ€™s configuration name, but they
...
Note:  Sometimes itâ€™s helpful to put the device type first, followed by the variant.
...
like: â€œinsert your joystick code hereâ€? as these comments typically arenâ€™t
